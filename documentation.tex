...
In order to make the code environments\sidenote{See section \href{sub:code}{code}.} and syntax highlighting work, it is needed to have Python\sidenote{Python has to be on the \texttt{PATH}.} installed on your system, along with the \texttt{pygments} package. With \texttt{pip} simply execute
\begin{codebox}{text}
...
To insert raw, unformatted text or graphics or whatever in the margin, use the command\\ \inlinecode{latex}{\marginpar{<unformatted margin text>}} and it will look like what appears here in the margin\marginpar{This is just unformatted text in the margin. It is in \texttt{\textbackslash normalsize}, which makes it stick out way too much.}. Note how it is the same size as the main text.
...
If for any reason a figure caption has to be put in the main text block, just use the regular \texttt{figure} environment. The following shortcut macros will also do. The result of \inlinecode{latex}{\plainfig} is shown in figure \ref{fig:figure-plain}.
...
\begin{tabularx}{\linewidth}{rXXXXXXrXXXXXX}
	...
	\bottomrule
\end{tabularx}
\sidecaption{Table des marées à Douarnenez du 23 au 26 juillet 2021, Service Hydrographique et Océanographique de la Marine, \url{maree.shom.fr}.\\This is a wide table, called with the \inlinecode{latex}{table*} environment. The caption is also a \inlinecode{latex}{\sidecaption{}}.\label{tab:table-wide}}
\end{table*}\FloatBarrier


To typeset the table \ref{tab:table-text}, use the following code, which is just a \inlinecode{latex}{table} environment with a \inlinecode{latex}{\sidecaption}. For table \ref{tab:table-wide}, use the \inlinecode{latex}{table*} environment with either \inlinecode{latex}{\sidecaption} or \inlinecode{latex}{\caption}.\inlinecode{latex}{\FloatBarrier} is there to make sure the floats appear in order.\sidenote[][100pt]{\inlinecode{latex}{\FloatBarrier} works for all floating environments, including figures.}
...
To produce tables in the margin like table \ref{tab:table-margin}, use the \inlinecode{latex}{margintable} environment like in the following.
